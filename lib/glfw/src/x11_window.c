...
static void drainEmptyEvents(void) {
  for (;;) {
    char dummy[64];
    const ssize_t result = read(_glfw.x11.emptyEventPipe[0], dummy,
                                sizeof(dummy));
...
//
static Bool
isSelectionEvent(Display* display, XEvent* event, XPointer pointer) {
...
//
static Bool
isFrameExtentsEvent(Display* display, XEvent* event, XPointer pointer) {
...
//
static Bool
isSelPropNewValueNotify(Display* display, XEvent* event, XPointer pointer) {
...
void _glfwPushSelectionToManagerX11(void) {
  ...
  for (;;) {
    ...
    while (XCheckIfEvent(_glfw.x11.display, &event, isSelectionEvent, NULL)) {
      switch (event.type) {
        case SelectionRequest:handleSelectionRequest(&event);
          break;
