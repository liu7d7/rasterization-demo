...
static const char* getEGLErrorString(EGLint error) {
  switch (error) {
    ...
    case EGL_CONTEXT_LOST:return "The application must destroy all contexts and reinitialise";
    default:return "ERROR: UNKNOWN EGL ERROR";
  }
}
...
GLFWbool _glfwCreateContextEGL(_GLFWwindow* window,
  ...
  if (_glfw.egl.KHR_create_context) {
    ...
    if (ctxconfig->noerror) {
      if (_glfw.egl.KHR_create_context_no_error) setAttrib(
        EGL_CONTEXT_OPENGL_NO_ERROR_KHR, GLFW_TRUE);
    }
    ...
    if (ctxconfig->client == GLFW_OPENGL_ES_API) setAttrib(
      EGL_CONTEXT_CLIENT_VERSION, ctxconfig->major);
  }
  ...
  if (fbconfig->sRGB) {
    if (_glfw.egl.KHR_gl_colorspace) setAttrib(EGL_GL_COLORSPACE_KHR,
                                               EGL_GL_COLORSPACE_SRGB_KHR);
  }

  if (!fbconfig->doublebuffer) setAttrib(EGL_RENDER_BUFFER, EGL_SINGLE_BUFFER);

  if (_glfw.egl.EXT_present_opaque) setAttrib(EGL_PRESENT_OPAQUE_EXT,
                                              !fbconfig->transparent);
