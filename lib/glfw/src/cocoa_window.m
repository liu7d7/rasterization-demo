...
static NSUInteger translateKeyToModifierFlag(int key) {
  switch (key) {
    ...
    case GLFW_KEY_RIGHT_SHIFT:return NSEventModifierFlagShift;
    case GLFW_KEY_LEFT_CONTROL:
    case GLFW_KEY_RIGHT_CONTROL:return NSEventModifierFlagControl;
    case GLFW_KEY_LEFT_ALT:
    case GLFW_KEY_RIGHT_ALT:return NSEventModifierFlagOption;
    case GLFW_KEY_LEFT_SUPER:
    case GLFW_KEY_RIGHT_SUPER:return NSEventModifierFlagCommand;
    case GLFW_KEY_CAPS_LOCK:return NSEventModifierFlagCapsLock;
  }
...
- (void)windowDidResize:(NSNotification*)notification {
  ...
  if (contentRect.size.width != window->ns.width ||
    ...
    _glfwInputWindowSize(window, contentRect.size.width,
...
static GLFWbool createNativeWindow(_GLFWwindow* window,
  ...
  _glfwPlatformGetFramebufferSize(window, &window->ns.fbWidth,
                                  &window->ns.fbHeight);
...
void _glfwPlatformGetWindowPos(_GLFWwindow* window, int* xpos, int* ypos) {
  @autoreleasepool {
    ...
    if (ypos)
      *ypos = _glfwTransformYNS(
        contentRect.origin.y + contentRect.size.height - 1);
...
void _glfwPlatformSetWindowPos(_GLFWwindow* window, int x, int y) {
  @autoreleasepool {
    ...
    const NSRect contentRect = [window->ns.view frame];
    const NSRect dummyRect = NSMakeRect(x, _glfwTransformYNS(
      y + contentRect.size.height - 1), 0, 0);
...
void _glfwPlatformSetWindowMonitor(_GLFWwindow* window,
  ...
  @autoreleasepool {
    ...
    if (window->monitor) {
      ...
      NSRect contentRect = NSMakeRect(xpos,
                                      _glfwTransformYNS(ypos + height - 1),
                                      width, height);
...
int _glfwPlatformWindowHovered(_GLFWwindow* window) {
  @autoreleasepool {
    ...
    return NSMouseInRect(point,
                         [window->ns.object convertRectToScreen:[window->ns.view frame]],
                         NO);
...
void _glfwPlatformSetCursorPos(_GLFWwindow* window, double x, double y) {
  @autoreleasepool {
    ...
    if (window->monitor) {
      ...
      const NSRect localRect = NSMakeRect(x, contentRect.size.height - y - 1, 0,
                                          0);
...
int _glfwPlatformCreateCursor(_GLFWcursor* cursor,
  ...
  @autoreleasepool {
    ...
    native = [[NSImage alloc] initWithSize:NSMakeSize(image->width,
                                                      image->height)];
    [native addRepresentation:rep];

    cursor->ns.object = [[NSCursor alloc] initWithImage:native
                                                hotSpot:NSMakePoint(xhot,
                                                                    yhot)];
